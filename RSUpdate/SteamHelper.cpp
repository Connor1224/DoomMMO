...
void CSteamHelper::InitSteam()
{
  ...
  if(ld == 0)
  {
	  r3dOutToLog("Failed to load steam_api.dll\n");
	  return;
  }
  ...
  if(!init)
  {
	  r3dOutToLog("Failed to init steam\n");
	  return;
  }
  ...
  if(!logged)
  {
	  r3dOutToLog("steam: user not logged in\n");
	  return;
  }
  ...
  authToken.append((BYTE*)ticketData, sizeTicket);
