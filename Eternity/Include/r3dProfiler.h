...
#define R3DPROFILE_START(n)		{ static uint32_t sHash=0; while(!sHash) sHash=r3dHash::MakeHash(n); r3dProfiler::StartSample(n, sHash);	}
#define R3DPROFILE_END(n)		{ static uint32_t sHash=0; while(!sHash) sHash=r3dHash::MakeHash(n); r3dProfiler::EndSample(n, sHash);		}
#define R3DPROFILE_FUNCTION(n)  static uint32_t sFuncHash=0; while(!sFuncHash) sFuncHash=r3dHash::MakeHash(n); r3dProfilerObject profileFunction(n,sFuncHash);
