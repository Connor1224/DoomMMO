...
{
	static std::vector<int> old2new;
	static std::vector<T> tempVerts ;
	...
	if( !vdest )
	{
		std::vector<T>().swap( tempVerts ) ;
		std::vector<int>().swap( old2new ) ;
...
void BuildPreTLOptimizeMap( std::vector<int>* oMap, int* idest, const int* indices, int NumVertices, int NumIndices );