...
REG_VAR_C( r_server_region,	-1,			-1,	0, VF_SAVE | VF_CONSTRAINT); // 0 - us west, 1-eu, 2 - any
...
REG_VAR( r_fullscreen_load		, false			, 0 );		// this value will hold r_fullscreen after loading ini file
...
REG_VAR( r_depth_screenshot_name,	"depth.jpg",	0 );
...
REG_VAR( r_screenshot_name,			"screen.jpg",	0 );
...
REG_VAR( g_FastLoad,	true,			0 );
REG_VAR( g_serverip,			"127.0.0.1",	0 ); // 127.0.0.1 - IP of dev server where we host dev master\supervisor
REG_VAR( g_api_ip,				"127.0.0.1", 0); // 127.0.0.1
