...
void ClientGameLogic::OnNetData(DWORD peerId, const r3dNetPacketHeader* packetData, int packetSize)
{
	...
	// r3dOutToLog("OnNetData from peer%d, obj:%d(%s) , id%d, event:%d\n", peerId, id, fromObj ? fromObj->Name.c_str() : "",evt->FromID, evt->EventID);
...
IMPL_PACKET_FUNC(ClientGameLogic, PKT_S2C_SendGroupIDChange)
{
}

IMPL_PACKET_FUNC(ClientGameLogic, PKT_S2C_Threat)
{
	hudMain->setThreatValue(n.value);
}
...
IMPL_PACKET_FUNC(ClientGameLogic, PKT_S2C_Flashlight)
{
}
...
{
	obj_Vehicle* vehi = (obj_Vehicle*)srv_CreateGameObject("obj_Vehicle", "Data\\ObjectsDepot\\Vehicles\\Drivable_Stryker.sco", n.pos);
	vehi->DisablePhysX = false;
	vehi->setVehicleSpawner(0);
...
IMPL_PACKET_FUNC(ClientGameLogic, PKT_S2C_PlayerNameJoined)
{
    ...
    playerNames[n.peerId].isLegend = (n.flags & 2)?true:false;
    playerNames[n.peerId].isDev = (n.flags & 3)?true:false;
    playerNames[n.peerId].isPunisher = (n.flags & 4)?true:false;
...
int ClientGameLogic::ProcessWorldEvent(GameObject* fromObj, DWORD eventId, DWORD peerId, const void* packetData, int packetSize)
{
	...
	switch(eventId) 
	{
		...
		//	DEFINE_PACKET_HANDLER(PKT_S2C_Flashlight);
		DEFINE_PACKET_HANDLER(PKT_S2C_Threat);
