...
int HUDDisplay::Update()
{
	...
	//r3d_assert(plr);
	if(plr && plr != gClientLogic().localPlayer_ && plr->GroupID == gClientLogic().localPlayer_->CurLoadout.GroupID && plr->GroupID != 0 && !plr->CurLoadout.isGroupShow)
	{
	plr->CurLoadout.isGroupShow = true;
	char plrUserName[64]; plr->GetUserName(plrUserName);
	hudMain->addplayertogroup(plrUserName,false);
	}
	}*/
	int value;
...
void HUDDisplay::eventPlayerListAction(r3dScaleformMovie* pMove, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	{
		if(action == 2) // Accept Group Invite
		{
			int index = 0;
		...
		if(action == 3) // Teleport To Player
		{
			showChatInput();
		...
		if(action == 4) // Teleport Player to You
		{
			showChatInput();
	...
	{
		if(action == 2) // Send Player Group Invite
		{
			const ClientGameLogic& CGL = gClientLogic();
		...
		if(action == 3) // Accept Group Invite
		{
			int index = 0;
			for(int i=0; i<R3D_ARRAYSIZE(gClientLogic().playerNames); i++)
			{
		...
		if(action == 4) // Decline Group Invite
		{
			int index = 0;
			for(int i=0; i<R3D_ARRAYSIZE(gClientLogic().playerNames); i++)
			{
...
void HUDDisplay::eventChatMessage(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	if (gUserProfile.ProfileData.isDevAccount)
	{
		...
		if(strncmp(s_chatMsg, "/ban", 3) == NULL)
		{
			gUserProfile.ApiBan();
			r3dError("Banned");
		}
...
{
	if(!Inited) return;
	gfxHUD.SetVariable("_root.Main.PlayerDead.Text.text", "EXITING IN 123");
