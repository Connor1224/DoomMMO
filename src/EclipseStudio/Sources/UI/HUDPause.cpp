...
void HUDPause::eventContextMenu_Action(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	case 7:
		...
		{
			eventBackpackUseItem(slotID);
			g_pPhysicsWorld->m_VehicleManager->getRealDrivenVehicle()->fuel += 50.0f;
			g_pPhysicsWorld->m_VehicleManager->getRealDrivenVehicle()->fuel = R3D_MIN(g_pPhysicsWorld->m_VehicleManager->getRealDrivenVehicle()->fuel, 100.0f);
			d_drive_vehiclescon->SetBool(true);
			//SoundSys.Start(gClientLogic().localPlayer_->m_sndVehicleDrive);
		}
		else
		{
			Scaleform::GFx::Value var[3];
			var[0].SetString("ENTER IN VEHICLES TO REFUEL");
			var[1].SetBoolean(true);
			var[2].SetString("");
			gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
		}
...
void HUDPause::Activate()
{
	...
	var[6].SetNumber( 0 ); // not used
	var[7].SetNumber( g_tps_camera_mode->GetInt());
	var[8].SetNumber( 0 ); // not used
