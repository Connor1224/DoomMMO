...
namespace physx
{
	...
	inline PxU32 GetNbTorqueCurvePair( const PxVehicleEngineData* inStats ) { return inStats->mTorqueCurve.getNbDataPairs() * 2; }
    inline void SetTorqueCurvePair( PxVehicleEngineData* inStats, PxU32 index, PxReal inData)
	{  
		inStats->mTorqueCurve.mDataPairs[index] = inData;
	}
	inline PxReal GetTorqueCurvePair( const PxVehicleEngineData* inStats, PxU32 index)
	{ 
		return inStats->mTorqueCurve.mDataPairs[index];
	}

	PX_PHYSX_CORE_API MTorqueCurveProperty::MTorqueCurveProperty()
		: PxExtendedIndexedPropertyInfo<PxVehiclePropertyInfoName::PxVehicleEngineData_MTorqueCurve
				, PxVehicleEngineData
				, PxU32
				, PxReal>("MTorqueCurve", GetTorqueCurvePair, GetNbTorqueCurvePair, SetTorqueCurvePair)
	{
	}
