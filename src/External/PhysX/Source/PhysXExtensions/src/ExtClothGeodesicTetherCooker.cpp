...
namespace
{
	...
	{		
		...
		int v0 = indices[e.mHalfEdgeIndex];
		int v1 = indices[e.mHalfEdgeIndex + offset1];
...
void PxClothGeodesicTetherCookerImpl::createTetherData(const PxClothMeshDesc &desc)
{
	...
	{
		mVertices[i] = *pIt++;
		mAttached[i] = wIt.ptr() ? (*wIt++ == 0.0f) : 0;
	}
	...
	for (PxU32 i = 0; i < mNumParticles; ++i)
	{
		...
		if (mAttached[i])
			vertexIslandHeap.pushBack(VertexDistanceCount(i, FLT_MAX, 0));
	}
	...
	while (!vertexIslandHeap.empty())
	{
		...
		// new cluster
		if (vertexIsland[vi.vertNr] == PxU32(-1))
		{
			islandFirst.pushBack(islandIndexCnt++);
			vertexIsland[vi.vertNr] = islandCnt++;
			vi.distance = 0;
			islandIndices.pushBack(vi.vertNr);
		}
		
		// for each adjacent vj that's not visited
		const PxU32 begin = (PxU32)valency[vi.vertNr];
		const PxU32 end = (PxU32)valency[vi.vertNr + 1];
		for (PxU32 j = begin; j < end; ++j)
		{
			...
			islandIndexCnt++;
			vertexIsland[vj] = vertexIsland[vi.vertNr];
			pushHeap(vertexIslandHeap, VertexDistanceCount(vj, vi.distance + 1.0f, 0));
		}
	}
	...
	for (PxU32 i = 0; i < islandCnt; i++)
	{
		...
		for (PxU32 j = beginIsland; j < endIsland; j++)
		{
			...
			vertexHeap.pushBack(VertexDistanceCount(vj, 0.0f, 0));
		}
		...
		while (!vertexHeap.empty())
		{
			...
			// for each adjacent vj that's not visited
			const PxI32 begin = valency[vi.vertNr];
			const PxI32 end = valency[vi.vertNr + 1];
			for (PxI32 j = begin; j < end; ++j)
			{
				const PxI32 vj = neighbors[j];
				PxVec3 edge = mVertices[vj] - mVertices[vi.vertNr];
	...
	for (PxU32 i = 0; i < mNumParticles; i++)
	{
		...
		{
			int parent = vertexParentBuffer[j * mNumParticles + i];
...
PxU32 PxClothGeodesicTetherCookerImpl::findTriNeighbors()
{
	...
	for (PxU32 i = 0; i < numTriangles; ++i)
	{
		...
		edges.pushBack(MeshEdge(i0, i1, 3*i));
		edges.pushBack(MeshEdge(i1, i2, 3*i+1));
		edges.pushBack(MeshEdge(i2, i0, 3*i+2));
	}
	...
	for(int i=0; i < numEdges; )
	{
		...
		while(++j < i)
		{
			const MeshEdge& e1 = edges[j];
			bool orientation1 = checkEdgeOrientation(e1, mIndices);
			mTriNeighbors[e0.mHalfEdgeIndex] = e1.mHalfEdgeIndex/3;
			mTriNeighbors[e1.mHalfEdgeIndex] = e0.mHalfEdgeIndex/3;
...
void PxClothGeodesicTetherCookerImpl::findVertTriNeighbors()
{
	...
	{
		vertTriangles.pushBack(VertTriangle(mIndices[3*i], i));
		vertTriangles.pushBack(VertTriangle(mIndices[3*i+1], i));
		vertTriangles.pushBack(VertTriangle(mIndices[3*i+2], i));
	}
...
int PxClothGeodesicTetherCookerImpl::computeVertexIntersection(PxU32 parent, PxU32 src, PathIntersection &path)
{
	...
	if (closestVert > 0)
	{
		path = PathIntersection(true, closestVert, (mVertices[src] - mVertices[closestVert]).magnitude());
		return 1;
	}
...
int PxClothGeodesicTetherCookerImpl::computeEdgeIntersection(PxU32 parent, PxU32 edge, float in_s, PathIntersection &path)
{
	...
	PxU32 e0 = mIndices[tid*3 + eid];
	PxU32 e1 = mIndices[tid*3 + (eid+1)%3];
	...
	if ( checkEdge(i0, i1, e0, e1)) {
		eid = 0;
	}
	else if ( checkEdge(i1, i2, e0, e1)) {
		eid = 1;
		PxU32 tmp = i2;
		i2 = i0;
		i0 = i1;
...
float PxClothGeodesicTetherCookerImpl::computeGeodesicDistance(PxU32 i, PxU32 parent, int &errorCode)
{
	...
	PxVec3 parentPos = mVertices[parent];
	PxVec3 vertPos = mVertices[i];
