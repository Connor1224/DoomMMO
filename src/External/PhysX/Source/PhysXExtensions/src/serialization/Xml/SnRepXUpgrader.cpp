...
namespace physx { namespace Sn {
	...
	RepXCollection& RepXUpgrader::upgrade10CollectionTo3_1Collection(RepXCollection& src)
	{
		...
		for ( const RepXCollectionItem* item = src.begin(), *end = src.end(); item != end; ++ item )
		{
			...
			editor.setNode( *const_cast<XmlNode*>( newItem.mDescriptor ) );
	...
	RepXCollection& RepXUpgrader::upgrade3_1CollectionTo3_2Collection(RepXCollection& src)
	{
		...
		for ( const RepXCollectionItem* item = src.begin(), *end = src.end(); item != end; ++ item )
		{
			...
			if ( strstr( newItem.mLiveObject.mTypeName, "PxMaterial" ) )
			{
	...
	RepXCollection& RepXUpgrader::upgrade3_2CollectionTo3_3Collection(RepXCollection& src)
	{
		...
		struct RenameSpringToStiffness
		{
			RenameSpringToStiffness(XmlReaderWriter& editor): mEditor(editor) {}

			void traverse()
			{
				mEditor.pushCurrentContext();

				// a known property of the hierarchy: if we find a parameter named "spring" we don't need to recurse
				if(!mEditor.renameProperty("Spring", "Stiffness"))
				{				
		...
		private:
			RenameSpringToStiffness& operator=(const RenameSpringToStiffness&){return *this;}
		};
		...
		for ( const RepXCollectionItem* item = src.begin(), *end = src.end(); item != end; ++ item )
		{
			...
			if ( strstr( newItem.mLiveObject.mTypeName, "PxCloth" ) || strstr( newItem.mLiveObject.mTypeName, "PxClothFabric" ) )
			{  
			...
			if ( strstr( newItem.mLiveObject.mTypeName, "PxParticleSystem" ) || strstr( newItem.mLiveObject.mTypeName, "PxParticleFluid" ) )
			{
				editor.setNode( *const_cast<XmlNode*>( newItem.mDescriptor ) );
			...
			if(strstr( newItem.mLiveObject.mTypeName, "PxPrismaticJoint" ) 
			|| strstr(newItem.mLiveObject.mTypeName, "PxRevoluteJoint")
			|| strstr(newItem.mLiveObject.mTypeName, "PxSphericalJoint")
			|| strstr(newItem.mLiveObject.mTypeName, "PxD6Joint")
			|| strstr(newItem.mLiveObject.mTypeName, "PxArticulation"))
			{
				editor.setNode( *const_cast<XmlNode*>( newItem.mDescriptor ) );
