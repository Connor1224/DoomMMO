...
protected:
											PxBase(PxRefResolver& v)
											{
												mBaseFlags &= ~PxBaseFlag::eOWNS_MEMORY;
											    PX_UNUSED(v);
											}
											PxBase() : mConcreteType(PxConcreteType::eUNDEFINED), mBaseFlags(PxBaseFlag::eOWNS_MEMORY)
											{
											}
	

	PX_INLINE			void				setConcreteType(PxType t)								{ mConcreteType = t;								}
