...
namespace physx
{
	...
	*/
	...
	public:
		...
		/**
		\brief Deallocates the PxRepXSerializer instance.
		*/
		virtual void destroy() = 0;
		/**
		\brief The type this Serializer is meant to operate on.
		@see PxRepXObject::mTypeName
		*/
		...
		*/
		virtual void objectToFile( PxRepXObject inLiveObject, PxCollection* inCollection, XmlWriter& inWriter, MemoryBuffer& inTempBuffer, PxRepXInstantiationArgs& inArgs ) = 0;
	...
	*/
	...
	PX_INLINE PxRepXObject createRepXObject( const TDataType* inType, const PxRepXId inId )
	{
	...
	*/
	PX_INLINE PxRepXObject createRepXObject( const PxBase* inType, const PxRepXId inId )
	{
		PX_ASSERT( inType );
	...
	/**
	\brief Inline helper template function to create PxRepXObject form TDataType type using inType pointer as a PxRepXId id.
	*/	
	template<typename TDataType>
	PX_INLINE PxRepXObject createRepXObject( const TDataType* inType )
	{
		return createRepXObject( inType, static_cast<const PxRepXId>( reinterpret_cast<const size_t>( inType) ) );
	}
