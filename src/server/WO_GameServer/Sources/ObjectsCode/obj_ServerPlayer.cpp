...
void obj_ServerPlayer::OnNetPacket(const PKT_C2C_Aura_s& n)
{
	RelayPacket(&n, sizeof(n), true);
}
...
void obj_ServerPlayer::OnNetPacket(const PKT_C2S_FallingDamage_s& n)
{
	...
	// No falling damga for God or Spawn Protected players
	if (!profile_.ProfileData.isGod && !(loadout_->GameFlags & wiCharDataFull::GAMEFLAG_isSpawnProtected))
	{
...
void obj_ServerPlayer::OnNetPacket(const PKT_C2S_ValidateEnvironment_s& n)
{
	...
	if (fabs(n.CurTime - lastCurTime) < 0.01f && !firstTime) // 
	{
		...
		gMasterServerLogic.SendNoticeMsg("System Banned '%s'",userName);
...
BOOL obj_ServerPlayer::OnNetReceive(DWORD EventID, const void* packetData, int packetSize)
{
	...
	switch(EventID)
	{
		...
		DEFINE_GAMEOBJ_PACKET_HANDLER(PKT_C2C_Aura); // AomBE - Aura Packet
