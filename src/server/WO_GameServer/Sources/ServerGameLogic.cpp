...
bool ServerGameLogic::ApplyDamageToPlayer(GameObject* fromObj, obj_ServerPlayer* targetPlr, const r3dPoint3D& dmgPos, float damage, int bodyBone, int bodyPart, bool force_damage, STORE_CATEGORIES damageSource, int airState )
{
	...
	// Cannot damage same groups or same clan
	if(IsServerPlayer(fromObj))
	{
	...
	// can inflict damage to self (for instance from grenade splash damage)
...
void ServerGameLogic::UpdateNetObjVisData(DWORD peerId, GameObject* netObj)
{
	...
	if(nh->PeerVisStatus[peerId] == 0)
	{
		if (netObj->Class->Name != "obj_Building")
		{
			if(dist < nh->distToCreateSq) // ถ้าหากอยู่ในระยะ ส่?Packet ให้โหล?
			{
#ifdef _DEBUG			
				...
				else
				{
					...
					{
						nh->PeerVisStatus[peerId] = 1;
						peer.player->lastObjectLoad = r3dGetTime() + 0.00005f;
						int packetSize = 0;
						DefaultPacket* packetData = nh->NetGetCreatePacket(&packetSize);
						if(packetData)
						{
		...
		else
		{
			...
			//while(r3dGetTime() > peer.player->lastObjectLoad)
			//{
			if ((r3dGetTime() - peer.player->startPlayTime_) > 10.0f)
			{
				while (r3dGetTime() > peer.player->lastObjectLoad)
			 {
				 nh->PeerVisStatus[peerId] = 1;
				 peer.player->lastObjectLoad = r3dGetTime() + 0.00004f;
				 /*PKT_S2C_CreateBuilding_s n;
				 n.angle = netObj->GetRotationVector();
				 n.col = false;
				 if(netObj->ObjFlags & OBJFLAG_PlayerCollisionOnly)
	...
	{
		if(netObj && netObj->isObjType(OBJTYPE_Human)) // dont delete player
		{
		}
		else
		{
...
void ServerGameLogic::UpdateNetObjVisData(const obj_ServerPlayer* plr)
{
	...
	if (plr->isFastLoad)
	{
		...
		for(GameObject* obj=ServerDummyWorld.GetFirstObject(); obj; obj=ServerDummyWorld.GetNextObject(obj))
		{
			...
			if ((r3dGetTime() - plr->startPlayTime_) < 1.5f)
			{
				if ((plr->GetPosition() - obj->GetPosition()).LengthSq() < 400.0f && nh->PeerVisStatus[peerId] == 0)
					UpdateNetObjVisData(peerId, obj);
			}
			else
			{
...
IMPL_PACKET_FUNC(ServerGameLogic, PKT_C2S_Explosion_Damage)
{
	...
	ApplyDamage(fromObj, target, n.explosion_pos, damage, true, wc->category,isSpecial);
}
...
void ServerGameLogic::SendPacketZ(int value, DWORD peerId,obj_ServerPlayer* plr)
{
	PKT_S2C_Threat_s n1;
	n1.value = value;
	p2pSendToPeer(peerId, plr, &n1, sizeof(n1));
}
...
IMPL_PACKET_FUNC(ServerGameLogic, PKT_S2C_SendGroupIDChange)
{
	char find[128];
	r3dscpy(find, n.intogamertag);
	obj_ServerPlayer* tplr = FindPlayer(find);
...
IMPL_PACKET_FUNC(ServerGameLogic, PKT_S2C_SendGroupInvite)
{
	...
	if(tplr && plr)
	{
		...
		else
		{
			char message2[128] = {0};
			PKT_S2C_ReceivedGroupInvite_s n3;
			sprintf(message2, "Already Invited");
			r3dscpy(n3.text,message2);
			n3.peerId = 99999;
			//gServerLogic.p2pSendToPeer(peerId_, NULL, &n2, sizeof(n2));
			gServerLogic.p2pSendToPeer(plr->peerId_, plr, &n3, sizeof(n3));
...
IMPL_PACKET_FUNC(ServerGameLogic, PKT_S2C_Flashlight)
{
	char find[128];
	r3dscpy(find, n.plrname);
	obj_ServerPlayer* tplr = FindPlayer(find);
	if (tplr)
	{
		PKT_S2C_Flashlight_s n1;
		n1.peerId = tplr->peerId_;
		n1.Status = n.Status;
		r3dscpy(n1.plrname,n.plrname);
...
void ServerGameLogic::OnPKT_C2S_ScreenshotData(DWORD peerId, const int size, const char* data)
{
	...
	sprintf(chatmessage, "System Banned '%s' ",peer.player->loadout_->Gamertag);
...
int ServerGameLogic::ProcessWorldEvent(GameObject* fromObj, DWORD eventId, DWORD peerId, const void* packetData, int packetSize)
{
	...
	switch(eventId) 
	{
		...
		DEFINE_PACKET_HANDLER(PKT_S2C_SendGroupIDChange);
...
void ServerGameLogic::Tick()
{
	...
	if(curTime > nextDebugLog_) 
	{
		...
		r3dOutToLog("time: %.0f, plrs:%d/%d, net_lastFreeId: %d, objects: %d, ServerDummyobjects: %d , async:%s\n", 
