...
void ReloadMeshForEmitter(r3dString &path, ValueTracker<r3dString> &)
{
	...
	ParticleReloadMesh(*EditTorch->PType[curEmitterSlot], buf);
}

void ReloadTextureForEmitter(r3dString &path, ValueTracker<r3dString> &)
{
	if (path.Length() == 0)
	{
		EditTorch->PType[curEmitterSlot]->MeshTexture = 0;
		return;
	}
	...
	sprintf(buf, PARTICLE_DATA_PATH_PATTERN, path.c_str());
	if (curEmitterSlot < r3dParticleData::MAX_EMITTER_SLOTS && curEmitterSlot >= 0)
		ParticleReloadTex(&EditTorch->PType[curEmitterSlot]->MeshTexture, buf);
}

void ReloadDistortTextureForEmitter(r3dString &path, ValueTracker<r3dString> &)
{
	if (path.Length() == 0)
	{
		EditTorch->PType[curEmitterSlot]->MeshDistortTexture = 0;
		return;
	}
	...
	sprintf(buf, PARTICLE_DATA_PATH_PATTERN, path.c_str());
	if (curEmitterSlot < r3dParticleData::MAX_EMITTER_SLOTS && curEmitterSlot >= 0)
		ParticleReloadTex(&EditTorch->PType[curEmitterSlot]->MeshDistortTexture, buf);
}
...
void InitParticleEmitterValueTrackers(r3dParticleEmitter &pe)
{
	...
	MeshTexFNameTracker.Init(pe.MeshTexFName); // texture
	MeshTexFNameTracker.SetOnUndoCallback(&ReloadTextureForEmitter);
	bMeshDisableCullingTracker.Init(pe.bMeshDisableCulling);
	bUseDistortionTextureTracker.Init(pe.bMeshUseDistortTexture);
	MeshDistortTexFNameTracker.Init(pe.MeshDistortTexFName); // Distortion tex
	MeshDistortTexFNameTracker.SetOnUndoCallback(&ReloadDistortTextureForEmitter);
...
void ParticleEmitterAddDelUndo::CreateAction()
{
	*pel = internalCopy;
	if (internalCopy)
	{
		...
		ReloadDistortTextureForEmitter(internalCopy->MeshDistortTexFName, dummy);
		ReloadTextureForEmitter(internalCopy->MeshTexFName, dummy);
...
void ParticleEmitterAddDelUndo::SetAsDeleteUndo()
{
	...
	internalCopy = new r3dParticleEmitter();
